...
class SecureMessaging:
    def __init__(self, root, user_name, encrypt, decrypt):
        self.root = root
    ...
    def receive_encrypted(self, encrypted_msg, urgent=False):
        if urgent:
            self.urgent_messages.addFront(encrypted_msg)
        else:  
    ...
    def decrypt_next(self):
        ...
        if not self.urgent_messages.isEmpty():
            encrypted_msg = self.urgent_messages.removeFront()
...
Person1_window = SecureMessaging(root1, Person1, encrypt_with_person2_key, decrypt_with_person1_key)
Person2_window = SecureMessaging(root2, Person2, encrypt_with_person1_key, decrypt_with_person2_key)
